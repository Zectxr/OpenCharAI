...
class AccountMethods:
    ...
    async def __update_settings(self, options: Dict, **kwargs: Any) -> Dict:
        ...
        for key, value in options.items():
            if key == "persona_override" and "character_id" in options:
                persona_overrides = settings.get("personaOverrides", {})
                persona_overrides[options["character_id"]] = value
                settings["personaOverrides"] = persona_overrides
            elif key == "model_preference":
                settings["modelPreferenceSettings"] = value
            elif key == "output_style":
                settings["outputStyleSettings"] = value
    ...
    async def fetch_model_preference(self, **kwargs: Any) -> Dict:
        settings = await self.fetch_my_settings(**kwargs)
        return settings.get("modelPreferenceSettings", {})

    async def set_model_preference(self, model_type: str, **kwargs: Any) -> bool:
        try:
            await self.__update_settings(
                {"model_preference": {"defaultModelType": model_type}},
                **kwargs,
            )
            return True
        except Exception:
            raise SetError("Cannot set model preference.")

    async def fetch_output_style(self, **kwargs: Any) -> Optional[Dict]:
        settings = await self.fetch_my_settings(**kwargs)
        return settings.get("outputStyleSettings", None)

    async def set_output_style(self, style: str, **kwargs: Any) -> bool:
        try:
            await self.__update_settings(
                {"output_style": {"style": style}},
                **kwargs,
            )
            return True
        except Exception:
            raise SetError("Cannot set output style.")
